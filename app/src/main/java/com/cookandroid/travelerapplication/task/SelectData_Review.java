...
public class SelectData_Review extends AsyncTask<String,Void,String> { // 통신을 위한 InsertData 생성
    ProgressDialog progressDialog;
    private static String TAG = "youn"; //phptest log 찍으려는 용도

    public ArrayList articleArrayList;

    public <T> SelectData_Review(ArrayList<T> articleArrayList) {
    ...
    protected String doInBackground(String... params) {
        ...
        try{ // HttpURLConnection 클래스를 사용하여 POST 방식으로 데이터를 전송한다.
            ...
            Log.d(TAG, "SelectData_Review: Error",e);
    ...
    private void parseJSONArray(String result) throws JSONException {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            review.setReview_id(jsonObject.getString("review_id"));
            review.setCreated_date(jsonObject.getString("created_date"));
            review.setModified_date(jsonObject.getString("modified_date"));
            review.setDetailed_review(jsonObject.getString("detailed_review"));
            review.setRating(jsonObject.getString("rating"));
            review.setSolo_friendly_rating(jsonObject.getString("solo_friendly_rating"));
            review.setVisit_times(jsonObject.getString("visit_times"));
            review.setPlace_id(jsonObject.getString("place_id"));
            review.setUser_id(jsonObject.getString("user_id"));
            review.setNickname(jsonObject.getString("nickname"));
            review.setImage_url(jsonObject.getString("image_url"));
