...
public class SelectData_Travel_Count extends AsyncTask<String,Void,String> { // 통신을 위한 InsertData 생성
    ProgressDialog progressDialog;
    private static String TAG = "youn"; //phptest log 찍으려는 용도

    public ArrayList articleArrayList;

    public <T> SelectData_Travel_Count(ArrayList<T> articleArrayList) {
    ...
    protected String doInBackground(String... params) {
        ...
        try{ // HttpURLConnection 클래스를 사용하여 POST 방식으로 데이터를 전송한다.
            ...
            Log.d(TAG, "SelectData_Travel_Count: Error",e);
    ...
    private void parseJSONArray(String result) throws JSONException {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            String visited_count = jsonObject.getString("visited_count");
            String not_visited_count = jsonObject.getString("not_visited_count");
            travelCount.setVisited_count(visited_count);
            travelCount.setNot_visited_count(not_visited_count);

            articleArrayList.add(travelCount);
